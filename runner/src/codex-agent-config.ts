...
export function materializeCodexAgents(
  ...
  if (options.readOnlyHookCommand
    && successful.some(({ def }) => def.workspacePolicy === "read-only")) {
    // Temp hooks are vetted and generated by Hanoman; without this one-shot trust override Codex
    // skips them pending an interactive /hooks review, silently removing the safety boundary.
