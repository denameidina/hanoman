...
| Continue (SPEC-172) | Rencana satu fase: Execute. Amandemen 2026-09-23 (audit S3): berkas fase tak dikosongkan, jadi fase di rencana continue dikecualikan dari `doneAtBirth` (`CreateOpts.rerunPhases`) agar ⚠ tetap bisa menyala, dan prompt continue orchestrator menyatakan baris `Execute done` lama tak berlaku — orchestrator menulis baris BARU sesudah agen Execute sesi ini selesai |
| Resume (SPEC-394) | Orchestrator menerima `resumeClause`, loop mulai dari `r.next`. Amandemen 2026-09-23 (audit S2): agen fase menerima blok `=== MELANJUTKAN ===` sendiri (`PhaseAgentContext.resume`: worktree dipakai ulang/dibangun ulang, fase tercatat, "baca git log/status, jangan membuat dokumen kedua") — subagent tak pernah melihat prompt orchestrator. Flow project: hanya bila worktree dipakai ulang |
