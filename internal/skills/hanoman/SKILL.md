...
- **Dua peran user** (SPEC-617/ADR-0110, memperluas ADR-0028): `User.role` = `admin` (operator — cookie = akses penuh, perilaku lama persis) atau `client` (portal baca-saja ber-scope project). `@default("admin")` **disengaja** — itulah yang membuat migrasi aman untuk instance yang sudah berjalan (setiap baris lama otomatis admin, nol backfill). Klien digerbangi **allowlist deny-by-default** `services/client-access.ts` (`GET|HEAD /portal/**` · `/help/**` · `POST /auth/logout` · `POST /auth/change-password`; sisanya **403**), dipasang di `onRequest` **sebelum** bypass `/sync` & `/help` supaya allowlist adalah pernyataan lengkapnya — endpoint baru tertutup bagi klien sampai sengaja dibuka, dan upgrade WebSocket ikut tergerbang. Scope project ditegakkan `ClientProjectAccess` (project bukan-miliknya → **404 yang sama** dengan yang tak ada; id item bukan jalan pintas). `User.disabled` ditegakkan di **DUA** titik — login **dan** `lookupSession()` (sesi hidup 7 hari). `DELETE /auth/users/:id` menjaga **admin terakhir**, bukan user terakhir. `/portal` & `/client-accounts` = **COOKIE_ONLY**. Proyeksi portal = **allowlist field eksplisit** di `shared/src/portal.ts`, bukan `Omit<>`. **Gotcha Fastify:** hook `onRequest` ber-scope **tak berjalan** untuk path tanpa route (404 lahir dari not-found handler di luar scope), jadi "klien menembak path X → 404" tak membuktikan apa pun tentang gerbang — sifat baca-saja portal diuji lewat `app.hasRoute` + fungsi murni allowlist. Bootstrap: `POST /auth/setup` membuat akun pertama lalu tertutup (409). Lakukan `setup` segera pada deploy pertama.
