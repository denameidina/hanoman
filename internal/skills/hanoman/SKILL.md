...
- Dashboard: **React + TypeScript + Vite**. Server: **Node.js + TypeScript (Fastify)**. DB: **SQLite via Prisma 6** — satu berkas di `$HANOMAN_HOME` (default `~/.hanoman/hanoman.db`), **tanpa Docker/Postgres/Redis** (SPEC-398/ADR-0086). Lokasi data ditentukan fungsi-fungsi murni di `runner/src/paths.ts` (`resolveHome`/`resolveDataDirs`/`resolveDbUrl`/`dbFilePath`), dipakai server **dan** CLI; `resolveDataDirs` menurunkan transkrip/upload/key SSH dari `$HANOMAN_HOME` supaya direktori itu benar-benar satu batas backup/restore (SPEC-846 — sebelumnya `vps-key.ts` menurunkannya sendiri dari `homedir()`). Presedensi DB `HANOMAN_DATABASE_URL` → `DATABASE_URL` → `<home>/hanoman.db`: `HANOMAN_DATABASE_URL` non-`file:` **melempar** dan menunjuk `hanoman migrate-from-postgres`, sedangkan `DATABASE_URL` non-`file:` **diabaikan** dengan peringatan `dbUrlNotice` (amandemen ADR-0086 — CLI global mewarisi shell apa pun, jadi var bernama generik itu hampir selalu milik project lain).
...
- Terminal server: **node-pty + tmux** (socket `-L hanoman`, `remain-on-exit on`); terminal web: **xterm.js** merender TUI Claude Code apa adanya. tmux menahan sesi hidup lintas restart API (ADR-0016). xterm mengirim satu frame WS per ketikan, jadi route terminal memakai quota 6.000 frame/menit (bukan default 120); `TerminalPane` menahan input selama `CONNECTING` dan swipe vertikal satu jari menggulir scrollback lewat `Terminal.scrollLines()` tanpa menggerakkan layout (SPEC-771). **Arah keluarnya di-coalesce dan dikompresi** (SPEC-812, tanpa ADR — ADR-0014/0016 ditegakkan): node-pty membaca dengan buffer tetap **1024 byte**, jadi satu frame per chunk berarti **±128 frame/dtk ≈ 966 kbit/dtk per pane** saat sesi ramai keluaran — dikali jumlah pane di grid Terminal, tanpa kompresi (default `ws`), lewat tunnel ke ponsel; echo ketikan lahir di belakang antrean itu dan delay-nya tumbuh mengikuti kedalamannya. `pty.ts` karena itu menahan keluaran dalam jendela **`COALESCE_MS` = 16** (satu frame animasi; atau langsung saat menembus `COALESCE_MAX_BYTES` = 64 KiB) dan `app.ts` menyalakan **`perMessageDeflate`** (level 6, `memLevel` 7 — rasio identik dengan 8, memori separuh; `threshold` bawaan 1 KiB menjaga frame kecil tak membayar). Terukur di server hidup pada aliran yang sama: **1 464 KB → 24,1 KB di kawat (60,6×)**, **±128 → 16 frame/dtk**. **Tiga gotcha:** (1) `pending` masuk ke `scrollback` **hanya di dalam `flushOutput`** — kalau tidak, replay scrollback ke klien yang baru attach menduplikasi byte yang masih menunggu siaran; (2) `flushOutput` wajib dipanggil di `drop()` **dan** sebelum frame `exit` — byte terakhir sebuah sesi lahir tepat sebelum kliennya dilepas; (3) `trimScrollback` memotong hanya saat melewati `MAX_SCROLLBACK + SCROLLBACK_SLACK` (memotong tiap chunk meratakan cons-string = **178 µs/chunk = 210 ms CPU per 10 dtk per pane**). Kontrol negatif yang menutup hipotesis lain: pada CPU throttle 6× antrean `WriteBuffer` xterm **0 ms** — klien bukan penyebabnya, jalur jaringan yang membedakan mobile dari desktop. **Arah MASUK diselesaikan SPEC-856** (tanpa ADR; SPEC-812 ditegakkan, nol perubahan server): sebelumnya sebuah huruf lahir sesudah round-trip penuh — terukur **242 ms keydown→glyph pada RTT 200 ms di TUI claude, 2,7 ms sesudahnya (90×)**, bahkan di localhost 30,6 → 0,4 ms. Mekanismenya **echo prediktif lokal** di modul murni `src/src/screens/terminal-predict.ts` dengan **satu invarian**: begitu satu byte pun datang dari pty, prediksi di-rollback **sebelum** byte itu ditulis, dalam **satu** `term.write` — layar sesudah tiap frame server karena itu byte-identik dengan tanpa prediksi (dibuktikan model layar xterm **dan** `capture-pane` identik, teks muncul persis 1×). Yang membalik premis brief: satu keystroke membalas **1 byte di bash tetapi 1 540 byte repaint layar penuh di TUI claude**, dan repaint itu **ber-posisi absolut** + `\x1b[K` — jadi prediksi paling aman justru di tempat yang brief suruh matikan, dan gerbangnya **perilaku terukur, bukan nama program**. Gerbang mati: sakelar operator, socket belum open, alternate screen, input bukan teks tunggal, dua kolom terakhir, ekor baris tak kosong, baris berpola password, dan satu prediksi yang mencapai TTL 500 ms tanpa ter-echo (suspend 30 dtk — `read -s` dan tombol yang ditelan dialog sama-sama membalas **nol byte**, tak bisa dibedakan dari jaringan lambat kecuali lewat TTL). **Empat gotcha:** (1) gerbang alt-screen hanya `?1049`/`?1047` — `?47h` dan `?2004h` terukur ikut lahir dari handshake attach tmux **pada `bash` polos**, mempercayainya mematikan prediksi selamanya di setiap attach, senyap; (2) `view.cursorX` dibaca hidup dari xterm dan **sudah** memuat karakter yang diprediksi — menambahkan `pending.length` lagi terhadap tepi baris menghitungnya dua kali; (3) penanda wajib netral SGR (`\x1b[4m`…`\x1b[24m`), `\x1b[m` merusak latar yang dipakai `\x1b[K` saat rollback; (4) **semua** jalur input keluar lewat batcher yang sama sejak SPEC-878 (`term.onData` boleh ditahan 16 ms selagi prediksi aktif; clipboard/tap dialog/lampiran/papan tombol memakai `sendRaw` = `push(d, false)` yang menguras antrean lebih dulu lalu lewat utuh dalam satu frame) — jaminan SPEC-289/452/800/816 tak berubah, dan transposisi terukur `["\x1b","z"]` hilang secara konstruksi. Batcher **tak memanen ketikan manusia** (jeda 120–200 ms ≫ 16 ms); nilainya di burst — frame masuk 10 → 5, byte keluar 20 524 → 11 370, karena TUI agen menggambar ulang sekali per *event input*, bukan per karakter. Sakelarnya `hn.ui.v1.terminal.predict` (ADR-0115), default hidup. **Arah masuk disempurnakan SPEC-878/ADR-0134:** gerbang prediksi & TTL-nya dulu membaca `ws.readyState === OPEN` (fakta transport) sebagai fakta pengiriman — terukur, socket `OPEN` tanpa byte mengalir membuat 9 glyph muncul lalu **dihapus** (`\x1b[9D\x1b[K`) dan prediksi mati **30,5 dtk tanpa satu pun banner**, sementara socket tertutup memberi **0 tulis lokal untuk 14 keystroke** padahal bytenya aman diantre. Kontrak frame bertambah `{t:"in", d, seq}` → `{t:"ack", seq}` (`seq` opsional, merosot aman di rilis campuran); `View.connected` jadi `View.deliverable`; jam TTL berjalan **hanya** sesudah `onDelivered`, yaitu saat semua frame sudah diakui. Antrean outage berbatas **4 KiB**, terlihat di strip, tak pernah disalip byte baru, `resize` mendahului kurasannya, dan ia **tak pernah mengirim `\r`/`\n` tanpa operator menekan `Kirim`** — `capture-pane` membuktikan blob lama benar-benar men-submit baris yang salah ke agen. `isTerminalResponse` pindah ke `@hanoman/shared` supaya klien bisa menolak mengantrekan balasan handshake milik sambungan yang sudah mati (blob campuran menembus gerbang SPEC-860 apa adanya).
...
- **Skill library** (ADR-0172): skill dari empat lapis — `hanoman` (`$HANOMAN_HOME/skills`, **disuntik ke SETIAP sesi agen di semua project**), `user` (`~/.claude|~/.codex|~/.agents/skills`), `plugin` (baca-saja; fork untuk menyunting), `project` (folder berisi `SKILL.md` di checkout utama; `.claude/skills`→claude, `.agents/skills`/`.codex/skills`→codex, lainnya tak dimuat otomatis). REST `/api/skills*` (key `layer~projectId|-~source~name`, tulis ber-`baseHash` → 409), capability `skills:read|write` menurut method, MCP `hanoman_skills_list`/`hanoman_skill_read`/`hanoman_skill_write`/`hanoman_skill_create`/`hanoman_skill_fork`/`hanoman_skill_entry`/`hanoman_skill_delete` (danger). Penyuntikan fail-open (`server/src/services/skill-inject.ts`): claude `--add-dir` (argumen terakhir, worktree tak disentuh), codex symlink + exclude bertanda `# hanoman:skill-inject` di common dir. Skill project bernama sama menang.
...
- **Obrolan portal klien** (SPEC-854/[ADR-0129](../../docs/adr/0129-mesin-chat-portal-klien.md) · kuota [ADR-0130](../../docs/adr/0130-kuota-chat-portal-klien.md)): klien punya percakapan yang dijawab hanoman sendiri — dua tipe sesi (`brainstorm` → PRD draft, `tanya` → jawaban langsung), beda dari Help Center yang antrean tiket ke manusia. Tiap giliran = **proses `claude -p` berumur pendek** di **workspace dokumen** (direktori temp berisi hanya proyeksi portal project itu — tanpa source code), ber-`--tools "Read,Glob,Grep"`, **tanpa** flag bypass/MCP/skill/settings, **tanpa tmux/PTY**. Empat lapis penjagaan, semuanya fungsi murni: gerbang masukan (pesan klien jadi blok bahan ber-nonce) · workspace (allowlist berkas) · argv (flag wajib-ada & wajib-tak-ada) · gerbang keluaran (tolak kode/path/email/istilah teknis/**nama project lain**). Teks penolakan selalu **dikarang server**. Fail closed di production tanpa sandbox — **kebalikan** jalur pty. **Khusus claude**: `--tools` tak punya padanan di `codex exec`. Jatah bulanan per (project × tipe), satuannya sesi yang LAHIR.
...
- Prompt sesi memetakan fase → skill superpowers: Brainstorm→brainstorming, Audit→systematic-debugging, Plan→writing-plans, Execute→executing-plans + TDD + verification-before-completion (agen fase claude: subagent-driven-development, ADR-0170).
