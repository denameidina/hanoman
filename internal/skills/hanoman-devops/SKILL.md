...
- **Gerbang manusia yang sebenarnya = GitHub Environment `release` + Required reviewers.** Mendorong tag saja bukan gerbang: agen yang punya akses push bisa membuat tag. Reviewer-nya **dicabut 2026-07-31**, jadi saat ini tak ada gerbang manusia di jalur publish.
- **Commit yang ditag wajib sudah masuk `main`** (SPEC-851). `scripts/assert-release-ancestry.sh` berjalan tepat sesudah checkout, sebelum toolchain/install/build/OIDC. Ini bukan pengganti gerbang manusia — ia menjawab "kode ini sudah di branch rilis?", bukan "manusia setuju?". Butuh `fetch-depth: 0` (di repo dangkal `merge-base` menolak commit yang sebenarnya ADA di `main`, jadi skripnya fail closed) dan fetch `origin/main` eksplisit (checkout pada push bertag hanya mengambil refspec tag itu).
...
- **`HANOMAN_DATABASE_URL harus URL SQLite file:` saat boot atau migrasi** = knob milik hanoman menunjuk Postgres. Itu **disengaja** (amandemen ADR-0086: di knob sendiri niatnya eksplisit) — jangan diakali dengan menghapus pemeriksaannya; kosongkan var itu (default `$HANOMAN_HOME/hanoman.db`) atau selesaikan migrasinya. `DATABASE_URL` non-`file:` **tidak** memicu ini: ia diabaikan.
- **Baris log ``hanoman: DATABASE_URL bertipe `postgresql` DIABAIKAN``** (`start`/`doctor`/boot server) = env masih mewarisi Postgres, dan hanoman boot ke `$HANOMAN_HOME/hanoman.db`. Bukan galat, tapi satu-satunya sinyal bahwa instance memakai DB default — instance yang tampak kosong sesudah cutover hampir selalu ini. Selesaikan migrasinya, atau tunjuk berkasnya lewat `HANOMAN_DATABASE_URL=file:…` / `--db`.
- **`migrate-from-postgres` menarget berkas yang tak dimaksud** = target diturunkan dari env dan `DATABASE_URL` Postgres lama diabaikan, bukan ditolak. Sebut target eksplisit `--to /srv/hanoman-prod/hanoman.db`; `--to` melewati resolusi env sepenuhnya.
