...
| `hnm-hero-rig-{body,arm-ul,arm-ur,arm-ml,arm-mr}-v01.webp` (badan 407×487; lengan atas ±343×430, tengah ±405×199) | `master/hnm-hero-rig-*-master-v01.webp` | **rig hero ber-sendi** layar Dalang Hanoman: badan tanpa lengan atas (cakram pivot emas di bahu) + 4 lengan terpisah masing-masing memegang gapit **TANPA rantai/benang tergambar** — benang 100 % SVG dari aplikasi (keputusan: satu bahasa visual untuk tali; v1 ber-rantai diganti via edit `prompts/rig-nostrings.md` atas sheet v1). Dirakit CSS absolut dalam kanvas 835×582 unit dengan **ARM_SCALE 0.78** (generasi menggambar lengan terlalu tebal relatif badan; dikoreksi murni di geometri CSS, aset utuh), tiap lengan beranimasi `rotate` pada `transform-origin` = pusat cakramnya. Prompt `prompts/rig-sheet.md` (sheet 1536×1024, grid 3×2 sel 512); sheet sumber green-screen diarsip `master/hnm-hero-rig-sheet-master-v02.webp` (q90 — PNG codex tidak dikomit). Seluruh geometri (posisi %, origin sendi, jangkar tangan `[data-hand]`) diukur otomatis dari centroid piksel emas per kuadran — regenerasi: `python internal/scripts/dalang/rigbuild.py <sheet> <outdir> 0.78` dan tempel blok CSS keluarannya |
