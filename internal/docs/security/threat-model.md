...
| sibling subdomain membuka terminal WS atau token bocor ke log | exact Origin; 30s one-use subprotocol ticket; no query token; payload/rate/connection caps; periodic (60s) + frame-triggered (≤1/s, background) revalidation — revoke closes 1008 on the next verdict; a failing check is not a verdict | access proxy/log masih sensitif; redaksi header dan log policy tetap wajib |
